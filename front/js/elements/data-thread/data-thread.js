	...
	render: function()
	{
		...
		return /* html */ `
			<div :class="'box bg-' + background">
				<e-ui-status-empty ot-if="!entries.length" :icon="emptyIcon" :title="emptyTitle" :description="emptyDescription" :background="0"></e-ui-status-empty>
				<div ot-for="entry in entries" :ot-key="entry.id" :class="state(entry)">
					...
					<e-ui-form-textarea ot-if="_reply && entry.replyable && !entry.reply" :placeholder="replyPlaceholder" :rows="2" :maxlength="2000" :counter="false" :background="above()" :_enter="replier(entry.id)"></e-ui-form-textarea>
				</div>
				<div ot-if="_send" class="composer">
					<e-ui-form-textarea :placeholder="placeholder" :rows="2" :maxlength="2000" :counter="false" :background="above()" :_enter="sender()"></e-ui-form-textarea>
				</div>
			</div>
		`;
	}
